...
function drawRenderLayer (ctx, layer) {
  ...
  if (layer.flipper && layer.inTransition){
    return drawFlipperLayer(ctx, layer);
  }
...
function getOffsets(layer){
  var topOffset = 0;
  var leftOffset = 0;

  function r(layer){
    topOffset += (layer.translateY || 0);
    leftOffset += (layer.translateX || 0);
...
function drawCanvasImg(layer){
  var frameOffsetY = layer.frame.y;
  var frameOffsetX = layer.frame.x;

  var backingStoreScale = layer.scale || window.devicePixelRatio;
  var backingStore = new Canvas(layer.frame.width, layer.frame.height, backingStoreScale);
  var backingContext = backingStore.getContext('2d');

  layer.translate(-frameOffsetX, -frameOffsetY);
  drawRenderLayer(backingContext, layer);
  layer.translate(frameOffsetX, frameOffsetY);

  // var canvasContent = backingStore._canvas.toDataURL();
  // var _img = new Image();
  // _img.src = canvasContent;

  return backingStore._canvas;
}

function findFlipperLayer(layer, type){
  var result;

  function find(layer){
    for (var i = 0; i < layer.children.length; i++){
      if (layer.children[i][type]){
        result = layer.children[i];
        break;
      }
      find(layer.children[i]);
    }
  }
...
function drawFlipperLayer(ctx, flipperLayer){

  // console.log('draw flipper', layer);

  var mode = flipperLayer.flipperMode;

  function transitionEndHandler(){
    document.getElementById('app').removeChild(flipperContainer);
    flipperLayer.modeSetter(mode.split('.')[0]);
    drawRenderLayer(ctx, flipperLayer);
  }

  var frontLayer = findFlipperLayer(flipperLayer, 'isFlipperFront');
  var backLayer = findFlipperLayer(flipperLayer, 'isFlipperBack');

  var frontLayerImg = drawCanvasImg(frontLayer);
  var backLayerImg = drawCanvasImg(backLayer);

  var targetLayer = frontLayer;
  var offsets = getOffsets(targetLayer);

  var flipperContainer = document.createElement('div');

  flipperContainer.id = 'flipperContainer';
  flipperContainer.style.position = 'absolute';
  flipperContainer.style.left = targetLayer.frame.x + offsets.leftOffset + 'px';
  flipperContainer.style.top = targetLayer.frame.y + offsets.topOffset + 'px';
  flipperContainer.style.width = targetLayer.frame.width + 'px';
  flipperContainer.style.height = targetLayer.frame.height + 'px';
  flipperContainer.style.transformStyle = 'preserve-3d';
  flipperContainer.style.perspective = '1900px';
  flipperContainer.style.zIndex = 10;

  frontLayerImg.style.position = 'relative';
  frontLayerImg.style.width = '100%';
  frontLayerImg.style.height = '100%';
  frontLayerImg.style.left = 0;
  frontLayerImg.style.top = 0;
  frontLayerImg.style.backfaceVisibility = 'hidden';
  frontLayerImg.style.background = 'white';

  backLayerImg.style.position = 'absolute';
  backLayerImg.style.width = '100%';
  backLayerImg.style.height = '100%';
  backLayerImg.style.left = 0;
  backLayerImg.style.top = 0;
  backLayerImg.style.backfaceVisibility = 'hidden';
  backLayerImg.style.transform = 'rotateY(180deg)';
  backLayerImg.style.zIndex = 100;
  backLayerImg.style.background = 'black';

  flipperContainer.appendChild(frontLayerImg);
  flipperContainer.appendChild(backLayerImg);
  ...
  flipperContainer.style.transform = flipperLayer.flipperTransition[preState].transform;
  flipperContainer.style.transition = flipperLayer.flipperTransition[postState].transition;
  setTimeout(function(){
    flipperContainer.style.transform = flipperLayer.flipperTransition[postState].transform;
  }, 0);
  ...
  } else if('onotransitionend' in flipperContainer || navigator.appName === 'Opera') {
  ...
  if (transitionEndEvent){
    flipperContainer.addEventListener(transitionEndEvent, transitionEndHandler, false);
  }
  else {
    setTimeout(transitionEndHandler, parseFloat(flipperContainer.style.transition.split(' ')[1]) * 1000);
  }

  document.getElementById('app').appendChild(flipperContainer);
