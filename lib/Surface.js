...
var Surface = React.createClass({
  ...
  detectSwipe: function() {
    var touchesArray = this._touchesArray;

    if (!touchesArray || touchesArray.length < 2){
      return;
    }

    var firstTouch = touchesArray[0];
    var lastTouch = touchesArray[touchesArray.length - 1];
    ...
    var touchXArray = touchesArray.map(function(i){
      return i.touchX;
    });
    var touchYArray = touchesArray.map(function(i){
      return i.touchY;
    });

    if (
        ((lastTouch.timeStamp - firstTouch.timeStamp) < durationThreshold) &&
        (Math.abs(firstTouch.touchX - lastTouch.touchX) > horizontalDistanceThreshold) &&
        (Math.abs(firstTouch.touchY - lastTouch.touchY) < verticalDistanceThreshold)
      ) {
      return {
        type: (firstTouch.touchX > lastTouch.touchX ? "swipeleft" : "swiperight")
      };
    }
  ...
  handleTouchStart: function (e) {
    ...
    var originalEventType = e.type;

    e.type = 'swipeleft';
    var swipeLeftTarget = hitTest(e, this.node, this.getDOMNode());
    e.type = 'swiperight';
    var swipeRightTarget = hitTest(e, this.node, this.getDOMNode());
    ...
    if (swipeLeftTarget){
      this._swipeLeftTarget = swipeLeftTarget;
      this._touchesArray = [this._getTouchParams(e)];
    }
    if (swipeRightTarget){
      this._swipeRightTarget = swipeRightTarget;
      this._touchesArray = [this._getTouchParams(e)];
    }
  ...
  handleTouchMove: function (e) {
    this.hitTest(e);
    this._touchesArray = this._touchesArray || [];
    this._touchesArray.push(this._getTouchParams(e));
  },
  ...
  handleTouchEnd: function (e) {
    ...
    var hitTarget;
    var swipeEvent = this.detectSwipe();

    var hitHandle = hitTest.getHitHandle(e.type);
    var swipeHandle = swipeEvent && hitTest.getHitHandle(swipeEvent.type);

    if (swipeEvent){
      if (this._swipeLeftTarget && this._swipeLeftTarget[swipeHandle]){
        this._swipeLeftTarget[swipeHandle](e);
        this._swipeLeftTarget = undefined;
      }
      if (this._swipeRightTarget && this._swipeRightTarget[swipeHandle]){
        this._swipeRightTarget[swipeHandle](e);
        this._swipeRightTarget = undefined;
      }
    }
