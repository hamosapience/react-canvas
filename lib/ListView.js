...
var ListView = React.createClass({
  ...
  getInitialState: function () {
    return {
      scrollTop: this.props.scrollOffset
    };
  },
  ...
  componentWillReceiveProps: function(nextProps){
    this.setState({
      scrollTop: nextProps.scrollOffset
    });
    this.scroller.scrollTo(0, nextProps.scrollOffset, true);
  },
  ...
  handleTouchStart: function (e) {
    if (this.scroller) {
      ...
      if (this.props.swipeItems){
        this._touches = [this._getTouchParams(e)];
      }
    }
  },
  ...
  handleTouchMove: function (e) {
    if (this.scroller) {
      e.preventDefault();
      if (this.props.swipeItems){
        if ( Math.abs(this._touches[0].touchY - this._getTouchParams(e).touchY) > 30 ) {
  ...
  getVisibleItemIndexes: function () {
    ...
    for (var index=0; index < itemCount; index++) {
      ...
      if (itemScrollTop <= -(this.props.style.height + 100)) {
        continue;
      }
